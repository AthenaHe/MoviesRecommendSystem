...
object ConnHelper extends Serializable{
  lazy val jedis = new Jedis("192.168.8.130")
  lazy val mongoClient = MongoClient( MongoClientURI("mongodb://192.168.8.130:27017/recommender") )
}
...
object StreamingRecommender {
  ...
  val MONGODB_STREAM_RECES_COLLECTION="StreamRecs"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ssc = new StreamingContext(sc, Seconds(2)) //批处理时间，相当于2秒中执行一次，一般是500ms以上

    import spark.implicits._

    implicit val mongoConfig = MongoConfig(config("mongo.uri"), config("mongo.db"))
    //加载电影相似度矩阵数据，把它广播出去
    val simNovieMatrix = spark.read
    ...
    val simMovieMatrixBroadCast = sc.broadcast(simNovieMatrix)

    //定义kafka连接参数
    val kafkaParam = Map(
      "bootstrap.servers" -> "192.168.8.130:9092",
      "key.deserializer" -> classOf[StringDeserializer],
      "value.deserializer" -> classOf[StringDeserializer],
      "group.id" -> "recommender",
      "auto.offeset.reset" -> "latest"
    )
  ...
  def saveDataToMongoDB(uid: Int, streamRecs: Array[(Int, Double)])(implicit mongoConfig: MongoConfig): Unit ={
    // 定义到StreamRecs表的连接
    val streamRecsCollection = ConnHelper.mongoClient(mongoConfig.db)(MONGODB_STREAM_RECES_COLLECTION)
