...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //模型参数选择，这个在ALSTrainer中实现找到最优化参数，非常重要
    ...
    val userRecs = preRatings
      ...
      .groupByKey()
      .map{
        case (uid, recs) => UserRecs( uid, recs.toList.sortWith(_._2>_._2).take(MAX_RECOMMENDATION).map(x=>Recommendation(x._1, x._2)) )
      }
      .toDF()
    ...
    val movieRecs = movieFeatures.cartesian(movieFeatures)
      ...
      .groupByKey()
      .map{
        case (mid, items) => MovieRecs( mid, items.toList.sortWith(_._2 > _._2).take(MAX_RECOMMENDATION).map(x => Recommendation(x._1, x._2)) )
      }
      .toDF()
    movieRecs.write
      .option("uri", mongoConfig.uri)
